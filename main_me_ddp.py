...
def train(rank, num_gpu, cfg: DictConfig):
    ...
    for epoch in range(train_config.max_num_epoch):
        ...
        if rank == 0:
            if epoch % train_config.stdout_interval == 0:
                log.info(
                    'epoch [{}] , train loss: {} '.format(epoch, train_loss))
                sw.add_scalar('loss', train_loss, epoch)
                sw.add_scalar('lr', tqdm_dis['lr'], epoch)
            ...
            if epoch % train_config.validation_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    sw.add_scalar('val_loss', dev_loss, epoch)
                    sw.add_scalar('eer', EER, epoch)

                    if EER < best_eer:
                        best_eer = EER
                        path = os.path.join(os.getcwd(), '{}_epoch_{}_eer_model.pth'.format(epoch, EER))
                        torch.save(model.module.state_dict(), path)
                        log.info('epoch [{}], Flash the EER , save to {}'.format(epoch,path))
                model.train()
...
def main(cfg: DictConfig):
    ...
    if cfg.mode == 'train':
        ...
        mp.spawn(train, nprocs=cfg.training.num_gpus, args=(cfg.training.num_gpus,cfg))
