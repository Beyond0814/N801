...
class ResNet(nn.Module):
    def __init__(self, num_nodes, enc_dim, resnet_type='18', nclasses=2):
        ...
        self.fc = nn.Linear(256 * 4, enc_dim)
        ...
        ################   custom    ################
        self.self_attention_aggregation = nn.Sequential(
    ...
    def forward(self, x):
        ...
        w = self.self_attention_aggregation(x)

        # 频域特征
        w1 = F.softmax(w, dim =-1)
        m1 = torch.sum(x * w1, dim=-1).squeeze(dim=-1)
        e_S = m1.transpose(1,2)

        # 时域特征
        w2 = F.softmax(w, dim =-2)
        m2 = torch.sum(x * w2, dim=-2).squeeze(dim=-2)
        e_T = m2.transpose(1,2)

        S_stats = self.attention(e_S.contiguous())
        T_stats = self.attention(e_T.contiguous())
        stats = torch.cat([S_stats, T_stats], dim=-1)
        feat = self.fc(stats)
        mu = self.fc_mu(feat)
