...
class model(nn.Module):
    def __init__(self, model_config, device):
        ...
        ####
        self.ssl_model = SSLModel(model_config, self.device)
    ...
    def forward(self, x):
        ...
        x = self.LL(x_ssl_feat)  # (bs,frame_number,feat_out_dim)   fine-tunning的线性层
        ...
        e_S = m.transpose(1, 2) + self.pos_S  # 这里为什么要加一个可训练参数？位置编码？位置信息对频域特征比较重要？
        ...
        e_T = m1.transpose(1, 2)  # 为什么这里没有加位置编码？
