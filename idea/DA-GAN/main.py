...
def train(rank, num_gpu, cfg: DictConfig):
    ...
    gen_config = cfg.gen_configuration
    ...
    generator = Model.Generator(gen_config).to(rank)
    mpd = Model.MultiPeriodDiscriminator().to(device)
    msd = Model.MultiScaleDiscriminator().to(device)
    ...
    msd = DDP(msd, device_ids=[rank], find_unused_parameters=True)

    if rank == 0:
        sw = SummaryWriter(os.getcwd())
        nb_params = sum([param.view(-1).size()[0] for param in generator.parameters()])
        log.info('nb_params: {}'.format(nb_params))
        log.info(generator)

    optim_g = torch.optim.AdamW(generator.parameters(), train_config.lr, betas=train_config.adam_beta)
    optim_d = torch.optim.AdamW(itertools.chain(msd.parameters(), mpd.parameters()),
                                train_config.lr, betas=train_config.adam_beta)
    scheduler_g = torch.optim.lr_scheduler.ExponentialLR(optim_g, gamma=train_config.lr_decay)
    scheduler_d = torch.optim.lr_scheduler.ExponentialLR(optim_d, gamma=train_config.lr_decay)
