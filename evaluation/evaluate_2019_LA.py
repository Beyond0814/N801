...
def calculate_tDCF_EER(cm_score, label):
    Pspoof = 0.05
    cost_model = {
    ...
    asv_score_file = '/pubdata/zhongjiafeng/ASVspoofLA/ASVspoof2019_LA_asv_scores/ASVspoof2019.LA.asv.eval.gi.trl.scores.txt'
    asv_data = np.genfromtxt(asv_score_file, dtype=str)
    asv_keys = asv_data[:, 1]
    asv_scores = asv_data[:, 2].astype(np.float)

    cm_keys = label
    cm_scores = cm_score.astype(np.float)
...
if __name__ == '__main__':
    type = 'dev'
    CM_score = '/home/zhongjiafeng/Model/SSL/model_only_FC_CM_scores_file_SSL_LA_dev.txt'
    if type == 'dev':
        ASV_score = '/pubdata/zhongjiafeng/ASVspoofLA/ASVspoof2019_LA_asv_scores/ASVspoof2019.LA.asv.dev.gi.trl.scores.txt'
    else :
        ASV_score = '/pubdata/zhongjiafeng/ASVspoofLA/ASVspoof2019_LA_asv_scores/ASVspoof2019.LA.asv.eval.gi.trl.scores.txt'
    dev_eer, dev_tdcf = calculate_tDCF_EER(CM_score,
                                           ASV_score,
                                           output_file=None,
                                           printout=False)
    print('t-DCF:{}, eer:{}'.format(dev_tdcf,dev_eer))