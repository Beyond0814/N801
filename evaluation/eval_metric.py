...
def eval_to_score_file(file, log):
    meta = np.genfromtxt(file, dtype=str)
    ...
    acc = compute_acc(threshold,bona_cm,spoof_cm, log)
    return eer_cm * 100, acc

def compute_acc(threshold, bona_cm, spoof_cm, log):
    bona_num = len(bona_cm)
    spoof_num = len(spoof_cm)
    bona_right_num = len(bona_cm[bona_cm > threshold])
    spoof_right_num = len(spoof_cm[spoof_cm < threshold])
    acc = (bona_right_num + spoof_right_num)/(bona_num + spoof_num)
    log.info('bonafide : {} | spoof : {}'.format(bona_num-bona_right_num,spoof_num-spoof_right_num))
    return acc * 100
...
if __name__ == '__main__':
    path = '/home/zhongjiafeng/Model/N801/outputs/2023-03-24/16-53-42/FMFCC_A_wav2vec-ASSIST-cn_training_score_file.txt'
    print(eval_to_score_file(path))